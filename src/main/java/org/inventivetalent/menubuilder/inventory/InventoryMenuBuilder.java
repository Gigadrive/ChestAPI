...
public class InventoryMenuBuilder extends MenuBuilder<Inventory> {
	...
	public InventoryMenuBuilder withEventHandler(InventoryEventHandler eventHandler) {
		try {
			ChestAPI.getInstance().inventoryListener.registerEventHandler(this, eventHandler);
	...
	public InventoryMenuBuilder onInteract(InventoryMenuListener listener, ClickType... actions) {
		...
		try {
			ChestAPI.getInstance().inventoryListener.registerListener(this, listener, actions);
		} catch (IllegalArgumentException e) {
			throw e;
		}
		return this;
	...
	@Override
	public void dispose() {
		ChestAPI.getInstance().inventoryListener.unregisterAllListeners(getInventory());
	}

	public void unregisterListener(InventoryMenuListener listener) {
		try {
			ChestAPI.getInstance().inventoryListener.registerListener(this, listener, ALL_CLICK_TYPES);
		} catch (IllegalArgumentException e) {
			throw e;
		}
	}
